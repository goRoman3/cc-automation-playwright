# BUG — Get Call PCI Data always 500s for a valid own-site call

- **ID**: P1-GET-CALL-PCI-DATA
...
```json
{
  ...
  "bogusCallId": "00000000-0000-4000-8000-000000000000"
}
```

## Endpoint chain

## Step 1 — Get Call Info (prove the call is valid + own-site)

### Get Call Info (prove the call is valid + own-site)
...
## Step 2 — Get Call PCI Data — valid own-site call

### Get Call PCI Data — valid own-site call

```
startedAt:     2026-08-29T09:41:17.227Z
latencyMs:     501
method:        GET
url:           https://developer1.callcabinet.com/api/media/call-details/call-pci/?callId=d71ac345-86a0-f111-9b33-6045bded66d5
pathname:      /api/media/call-details/call-pci/
...
## Step 3 — Get Call PCI Data — bogus call id (contrast)

### Get Call PCI Data — bogus call id (contrast)

```
startedAt:     2026-08-29T09:41:22.560Z
latencyMs:     265
method:        GET
url:           https://developer1.callcabinet.com/api/media/call-details/call-pci/?callId=00000000-0000-4000-8000-000000000000
pathname:      /api/media/call-details/call-pci/
query:         {"callId":"00000000-0000-4000-8000-000000000000"}
Content-Type:  (none)
sub-key hdr:   <redacted len=32>
request body:  (none)
--
...
200 with PCI metadata (or an empty/no-PCI response) for a valid call.

## Actual

Get Call Info → 200 (call is valid, own site). Get Call PCI Data (valid call) → **500** `""`. Bogus id → 500 `""`.
...
- **call invalid / not own-site** — ruled out — Get Call Info → 200 for the same id/key
- **"no PCI data" is just an empty 200** — ruled out — status is 500, body is a 500 exception payload
- **request never fired** — ruled out — raw capture: GET /api/media/call-details/call-pci/ → 500

## Downstream impact

PCI compliance metadata (card-data pause/resume, suppression periods) is unreadable via the API for any call.
...
Whether a call that actually has PCI events behaves differently (test call may have none).

## Recommended regression test

`calls-api.spec.ts` › "KNOWN BUG — Get Call PCI Data always 500s" pins it. Keep.

## Suggested bug-ticket wording

**Title**: `GET media/call-details/call-pci?callId={id}` returns 500 for a valid own-site call.

**Steps**: `GET calls/details/{callId}` → 200 (call valid). `GET media/call-details/call-pci?callId={callId}` → **500** `""`.
